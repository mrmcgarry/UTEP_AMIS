...
//.defer(d3.json, "../data/uky_201702100900_15m.json") // d3.json will automatically parse the json object so no callback is necessary in this case
.await(function(error, file1) { 	// Sets the callback to be invoked when all deferred tasks have finished, i.e., the callback function will execute after d3.csv and its callback are executed.
	...
	getDataList();
	//$('#dataSelStat').html("<b>Hai</b>");
	//$('#dataSelStat').html(" ");
	//$('#dataSelStat').html("Bai");

	var mymap = renderMap("mapContainer",0,0,2);
	
	getTrafficData("uky_201702100900_15m", mymap, "uky_201702100900_15m", countryCoordData, continentCoordData);
});

function updateVizMsg(msg, dataInfo)
{
	
	var container = $('#datavizMsg');
	
	if (dataInfo){
		var outputmsg = "";
		
		source = msg.substring(0,msg.indexOf("_"));
		timestamp = msg.substring(msg.indexOf("_")+1, msg.lastIndexOf("_"));
		duration = msg.substring(msg.lastIndexOf("_")+1, msg.length);
		...
		outputmsg = "Data obtained on "+month+"/"+day+"/"+year+" (mm/dd/yyyy) at "+hour+":"+minute+" from "+source.toUpperCase()+". Traffic was captured for "+duration+".";

		container.html("<p><b>"+outputmsg+"</b></p>");
		
		return 0;
	}
	
	container.html("<p><b>"+msg+"</b></p>");
	return 0;
}
...
{ 
	...
	if(data != null)
    {
		...
		$('#dataSel').val('uky_201702100900_15m').change();
		document.getElementById("dataSel").selectedIndex=0;
...
function getTrafficData(fileName, leafletMAP, previousDataset, countryCoordData, continentCoordData)
{
	...
	$('dataSelStat').html("<b>Loading data, please wait...</b>");
...
function makeTrafficLayer (data, map, prevdataset, countryCoordData, continentCoordData) {
	...
	{	/* Note: All variables declared without var are declared like this to allow deletion. */		
		...
		traffic = buildHash (jason,section,sectionLabel, coordinates, { getHistData: (histocheck)? true:false });
		...
		for (var y=0; y<data[matrix].length; y++){
			for (var x=0; x<data[matrix][y].length; x++){				
				results.push({ SrcLabel: data[matrixLabel][y], DstLabel: data[matrixLabel][x], Bytes: data[matrix][y][x],
					...
					SrcPort: { Bytes: (options.getHistData)? data['asAppByteHistMatrix'][y][x] : 'undefined', Ports: (options.getHistData)? data['appLbl'] : 'undefined' },
					DstPort: { Bytes: (options.getHistData)? data['asAppByteHistMatrix'][y][x] : 'undefined', Ports: (options.getHistData)? data['appLbl'] : 'undefined' },
					...
					SrcPort: { Bytes: (options.getHistData)?
										(
											(matrix === "continentMatrixByte")? 
												data['continentAppByteHistMatrix'][y][x]
												:data['countryAppByteHistMatrix'][y][x]
										)
										:'undefined',
							   Ports: (options.getHistData)? data['appLbl']:'undefined' },
...
function drawTrafficPaths ( trafficData, map, options ) {
	...
	//console.log(options);
	//var tuftelvl = $('#tuftelvlsel').val;
	$('#dataSelStat').html('<b>Loading, please wait...</b>');
	...
	for (var i = 0; i<trafficData.length; i++){
		...
		{	
			...
			pointsList.push([new L.LatLng(0, 0),
							 new L.LatLng(0, 0)]);
		}
		else{
